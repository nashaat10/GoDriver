...
import User from '../models/User'; // Import the User model (to check if the manager and driver exist)
import AppError from '../utils/appError'; // Error handling utility
import catchAsync from '../utils/catchAsync'; // Async error handling utility
...
export const createTask = catchAsync(async (req, res, next) => {
  ...
  if (!driver || driver.role !== 'driver') {
    return next(new AppError('No driver found with that ID or the user is not a driver', 404));
  }
...
export const getTaskById = catchAsync(async (req, res, next) => {
  const task = await Task.findById(req.params.id).populate('managerId driverId'); // Populate manager and driver details
...
export const updateTask = catchAsync(async (req, res, next) => {
  ...
  if (driverId) {
    const driver = await User.findById(driverId);
    if (!driver || driver.role !== 'driver') {
      return next(new AppError('No valid driver found with that ID', 404));
    }
  }
